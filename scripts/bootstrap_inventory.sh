...
# Pre-flight jq check
# jq is used to verify JSONL output files generated by this script.
# For example: jq -s '.' logs/bootstrap_*.jsonl
# This check ensures users can validate output without a separate install step.
if ! command -v jq >/dev/null 2>&1; then
    echo -e "${RED}Error: jq is not installed${NC}"
    echo ""
    echo "jq is required for parsing JSONL output files."
    echo ""
    echo "Install jq:"
