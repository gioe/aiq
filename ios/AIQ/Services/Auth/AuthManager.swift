...
class AuthManager: ObservableObject, AuthManagerProtocol {
    ...
    /// Exchange a Google identity token for AIQ tokens and establish the session.
    ///
    ...
    func loginWithGoogle(identityToken: String) async throws {
        ...
        let signpostID = signposter.makeSignpostID()
        let state = signposter.beginInterval("Auth.LoginWithGoogle", id: signpostID)
        let startTime = CFAbsoluteTimeGetCurrent()

        do {
            let response = try await authService.loginWithGoogle(identityToken: identityToken)
            ...
            logger.warning(
                ...
                "Google OAuth login failed after \(elapsed, format: .fixed(precision: 2))s: \(errorDesc, privacy: .public)"
            )
