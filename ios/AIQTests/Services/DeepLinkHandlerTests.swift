@testable import AIQ
import AIQSharedKit
import XCTest

/// Tests for AIQDeepLinkParser URL parsing functionality
final class DeepLinkHandlerTests: XCTestCase {
    var sut: AIQDeepLinkParser!
    var mockAnalytics: MockAnalyticsManager!

    override func setUp() {
        super.setUp()
        mockAnalytics = MockAnalyticsManager()
        sut = AIQDeepLinkParser(analyticsManager: mockAnalytics)
    }

    // MARK: - URL Scheme Tests - Test Results

    func testParseURLScheme_TestResults_ValidID() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/results/123"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .testResults(id: 123), "should parse test results with valid ID")
    }

    func testParseURLScheme_TestResults_LargeID() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/results/999999"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .testResults(id: 999_999), "should handle large IDs")
    }

    func testParseURLScheme_TestResults_InvalidID_NonNumeric() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/results/abc"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid for non-numeric ID")
    }

    func testParseURLScheme_TestResults_MissingID() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/results"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid when ID is missing")
    }

    func testParseURLScheme_TestResults_ExtraPathComponents() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/results/123/extra"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // Extra path components are tolerated but logged as warnings.
        // This lenient behavior allows forward compatibility while alerting developers
        // to potentially malformed links via os_log warnings.
        XCTAssertEqual(result, .testResults(id: 123), "should parse ID even with extra components (warning logged)")
    }

    func testParseURLScheme_TestResults_MultipleExtraPathComponents() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/results/456/extra/path/components"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // Multiple extra components should also be tolerated with a warning logged
        XCTAssertEqual(result, .testResults(id: 456), "should parse ID with multiple extra components (warning logged)")
    }

    func testParseURLScheme_ResumeTest_ExtraPathComponents() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/resume/789/extra"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // Extra path components on resume test should also be tolerated with warning
        XCTAssertEqual(result, .resumeTest(sessionId: 789), "should parse session ID even with extra components (warning logged)")
    }

    // MARK: - URL Scheme Tests - Resume Test

    func testParseURLScheme_ResumeTest_ValidSessionID() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/resume/456"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .resumeTest(sessionId: 456), "should parse resume test with valid session ID")
    }

    func testParseURLScheme_ResumeTest_LargeSessionID() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/resume/888888"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .resumeTest(sessionId: 888_888), "should handle large session IDs")
    }

    func testParseURLScheme_ResumeTest_InvalidSessionID_NonNumeric() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/resume/xyz"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid for non-numeric session ID")
    }

    func testParseURLScheme_ResumeTest_MissingSessionID() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/resume"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid when session ID is missing")
    }

    // MARK: - URL Scheme Tests - Settings

    func testParseURLScheme_Settings_Valid() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://settings"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .settings, "should parse settings route")
    }

    func testParseURLScheme_Settings_WithTrailingSlash() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://settings/"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .settings, "should parse settings with trailing slash")
    }

    func testParseURLScheme_Settings_WithExtraPath() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://settings/notifications"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // Extra path components are tolerated but logged as warnings.
        // This lenient behavior allows forward compatibility while alerting developers
        // to potentially malformed links via os_log warnings.
        XCTAssertEqual(result, .settings, "should parse settings even with extra components (warning logged)")
    }

    func testParseURLScheme_Settings_MultipleExtraPathComponents() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://settings/notifications/email/preferences"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // Multiple extra components should also be tolerated with a warning logged
        XCTAssertEqual(result, .settings, "should parse settings with multiple extra components (warning logged)")
    }

    // MARK: - URL Scheme Tests - Invalid Routes

    func testParseURLScheme_EmptyPath() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid for empty path")
    }

    func testParseURLScheme_UnrecognizedHost() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://unknown/path"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid for unrecognized host")
    }

    func testParseURLScheme_TestWithoutAction() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid for test without action")
    }

    func testParseURLScheme_TestWithInvalidAction() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/invalid/123"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid for unrecognized test action")
    }

    // MARK: - Universal Link Tests - Test Results

    func testParseUniversalLink_TestResults_ValidID() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/test/results/789"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .testResults(id: 789), "should parse universal link test results with valid ID")
    }

    func testParseUniversalLink_TestResults_LargeID() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/test/results/111111"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .testResults(id: 111_111), "should handle large IDs in universal links")
    }

    func testParseUniversalLink_TestResults_InvalidID_NonNumeric() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/test/results/def"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid for non-numeric ID in universal link")
    }

    func testParseUniversalLink_TestResults_MissingID() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/test/results"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid when ID is missing in universal link")
    }

    func testParseUniversalLink_TestResults_ExtraPathComponents() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/test/results/123/extra/path"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // Extra path components on universal links should also be tolerated with warning
        XCTAssertEqual(result, .testResults(id: 123), "should parse ID with extra components in universal link (warning logged)")
    }

    // MARK: - Universal Link Tests - Resume Test

    func testParseUniversalLink_ResumeTest_ValidSessionID() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/test/resume/321"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .resumeTest(sessionId: 321), "should parse universal link resume test with valid session ID")
    }

    func testParseUniversalLink_ResumeTest_LargeSessionID() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/test/resume/777777"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .resumeTest(sessionId: 777_777), "should handle large session IDs in universal links")
    }

    func testParseUniversalLink_ResumeTest_InvalidSessionID_NonNumeric() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/test/resume/invalid"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid for non-numeric session ID in universal link")
    }

    func testParseUniversalLink_ResumeTest_MissingSessionID() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/test/resume"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid when session ID is missing in universal link")
    }

    func testParseUniversalLink_ResumeTest_ExtraPathComponents() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/test/resume/456/extra"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // Extra path components on universal link resume should also be tolerated with warning
        XCTAssertEqual(result, .resumeTest(sessionId: 456), "should parse session ID with extra components in universal link (warning logged)")
    }

    // MARK: - Universal Link Tests - Settings

    func testParseUniversalLink_Settings_Valid() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/settings"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .settings, "should parse settings from universal link")
    }

    func testParseUniversalLink_Settings_WithTrailingSlash() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/settings/"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .settings, "should parse settings with trailing slash in universal link")
    }

    func testParseUniversalLink_Settings_WithExtraPath() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/settings/extra"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // Extra path components on universal links should also be tolerated with warning
        XCTAssertEqual(result, .settings, "should parse settings with extra components in universal link (warning logged)")
    }

    func testParseUniversalLink_Settings_MultipleExtraPathComponents() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/settings/notifications/email"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // Multiple extra components on universal links should also be tolerated with warning
        XCTAssertEqual(result, .settings, "should parse settings with multiple extra components in universal link (warning logged)")
    }

    // MARK: - Universal Link Tests - Invalid Routes

    func testParseUniversalLink_EmptyPath() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid for empty path in universal link")
    }

    func testParseUniversalLink_RootPath() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid for root path in universal link")
    }

    func testParseUniversalLink_UnrecognizedPath() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/unknown"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid for unrecognized path in universal link")
    }

    func testParseUniversalLink_TestWithoutAction() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/test"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid for test without action in universal link")
    }

    func testParseUniversalLink_TestWithInvalidAction() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/test/unknown/123"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid for unrecognized test action in universal link")
    }

    // MARK: - Development Domain Universal Link Tests

    func testParseUniversalLink_DevDomain_TestResults_ValidID() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://dev.a-iq-test.com/test/results/123"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .testResults(id: 123), "should parse test results from dev domain")
    }

    func testParseUniversalLink_DevDomain_ResumeTest_ValidSessionID() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://dev.a-iq-test.com/test/resume/456"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .resumeTest(sessionId: 456), "should parse resume test from dev domain")
    }

    func testParseUniversalLink_DevDomain_Settings() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://dev.a-iq-test.com/settings"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .settings, "should parse settings from dev domain")
    }

    func testParseUniversalLink_DevDomain_InvalidRoute() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://dev.a-iq-test.com/unknown"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid for unrecognized route on dev domain")
    }

    func testParseUniversalLink_DevDomain_EmptyPath() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://dev.a-iq-test.com/"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid for empty path on dev domain")
    }

    func testParseUniversalLink_DevDomain_TestResults_InvalidID() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://dev.a-iq-test.com/test/results/abc"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid for non-numeric ID on dev domain")
    }

    func testParseUniversalLink_DevDomain_EquivalentToProd() throws {
        // Given - same path on prod and dev domains
        let prodURL = try XCTUnwrap(URL(string: "https://a-iq-test.com/test/results/789"))
        let devURL = try XCTUnwrap(URL(string: "https://dev.a-iq-test.com/test/results/789"))

        // When
        let prodResult = sut.parseDeepLink(prodURL)
        let devResult = sut.parseDeepLink(devURL)

        // Then
        XCTAssertEqual(prodResult, devResult, "dev and prod domains should produce equivalent results")
        XCTAssertEqual(devResult, .testResults(id: 789))
    }

    // MARK: - Invalid URL Tests

    func testParse_InvalidScheme_HTTP() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "http://a-iq-test.com/test/results/123"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid for http scheme")
    }

    func testParse_InvalidScheme_Custom() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "custom://test/results/123"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid for custom scheme")
    }

    func testParse_InvalidHost_UniversalLink() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://other.app/test/results/123"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid for wrong host in universal link")
    }

    func testParse_InvalidHost_WWW() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://www.a-iq-test.com/test/results/123"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should return invalid for www subdomain")
    }

    // MARK: - Edge Case Tests

    func testParse_URLWithQueryParameters_URLScheme() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/results/123?source=notification"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // Query parameters should be ignored
        XCTAssertEqual(result, .testResults(id: 123), "should parse URL ignoring query parameters")
    }

    func testParse_URLWithQueryParameters_UniversalLink() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/test/results/456?utm_source=email"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // Query parameters should be ignored
        XCTAssertEqual(result, .testResults(id: 456), "should parse universal link ignoring query parameters")
    }

    func testParse_URLWithFragment_URLScheme() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/results/789#section"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // Fragments should be ignored
        XCTAssertEqual(result, .testResults(id: 789), "should parse URL ignoring fragments")
    }

    func testParse_URLWithFragment_UniversalLink() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/settings#notifications"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // Fragments should be ignored
        XCTAssertEqual(result, .settings, "should parse universal link ignoring fragments")
    }

    func testParse_URLWithLeadingZeros_URLScheme() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/results/00123"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // Leading zeros should be handled by Int parsing
        XCTAssertEqual(result, .testResults(id: 123), "should handle IDs with leading zeros")
    }

    func testParse_URLWithNegativeID_URLScheme() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/results/-123"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // Negative IDs are invalid - database IDs must be positive
        XCTAssertEqual(result, .invalid, "should reject negative IDs")
    }

    func testParse_URLWithZeroID_URLScheme() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/results/0"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // Zero is invalid - database IDs must be positive
        XCTAssertEqual(result, .invalid, "should reject zero as ID")
    }

    func testParse_URLWithNegativeSessionID_URLScheme() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/resume/-456"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // Negative session IDs are invalid
        XCTAssertEqual(result, .invalid, "should reject negative session IDs")
    }

    func testParse_URLWithZeroSessionID_URLScheme() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/resume/0"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // Zero is invalid - database IDs must be positive
        XCTAssertEqual(result, .invalid, "should reject zero as session ID")
    }

    // MARK: - Integer Overflow Edge Case Tests

    func testParse_URLWithIntegerOverflow_URLScheme() throws {
        // Given - value exceeding Int.max
        let url = try XCTUnwrap(URL(string: "aiq://test/results/99999999999999999999999999999"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // Integer overflow should return invalid (Int parsing fails)
        XCTAssertEqual(result, .invalid, "should reject integer overflow values")
    }

    func testParse_URLWithIntegerOverflow_UniversalLink() throws {
        // Given - value exceeding Int.max
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/test/resume/99999999999999999999999999999"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should reject integer overflow values in universal links")
    }

    // MARK: - Decimal Value Edge Case Tests

    func testParse_URLWithDecimalID_URLScheme() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/results/123.456"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // Decimal values should return invalid (Int parsing fails)
        XCTAssertEqual(result, .invalid, "should reject decimal IDs")
    }

    func testParse_URLWithDecimalSessionID_URLScheme() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/resume/456.789"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should reject decimal session IDs")
    }

    func testParse_URLWithDecimalID_UniversalLink() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/test/results/123.456"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should reject decimal IDs in universal links")
    }

    // MARK: - URL Encoding Edge Case Tests

    func testParse_URLWithEncodedSpaces_URLScheme() throws {
        // Given - ID with URL-encoded spaces
        let url = try XCTUnwrap(URL(string: "aiq://test/results/123%20456"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // URL-encoded spaces should fail Int parsing
        XCTAssertEqual(result, .invalid, "should reject IDs with encoded spaces")
    }

    func testParse_URLWithEncodedSlash_URLScheme() throws {
        // Given - ID with URL-encoded slash (%2F = /)
        // Note: URL.pathComponents behavior changed in iOS 18.4+
        // - iOS 18.3 and earlier: %2F becomes a path separator after decoding
        //   → pathComponents = ["results", "123", "456"] → testResults(id: 123)
        // - iOS 18.4+: %2F decodes within the component, preserving boundaries
        //   → pathComponents = ["results", "123/456"] → Int fails → .invalid
        let url = try XCTUnwrap(URL(string: "aiq://test/results/123%2F456"))

        // When
        let result = sut.parseDeepLink(url)

        // Then - behavior depends on iOS version
        let osVersion = ProcessInfo.processInfo.operatingSystemVersion
        if osVersion.majorVersion > 18 || (osVersion.majorVersion == 18 && osVersion.minorVersion >= 4) {
            // iOS 18.4+: encoded slash decodes within component
            XCTAssertEqual(result, .invalid, "encoded slash decodes within component, not as path separator")
        } else {
            // iOS 18.3 and earlier: encoded slash becomes path separator
            XCTAssertEqual(result, .testResults(id: 123), "encoded slash becomes path separator on iOS < 18.4")
        }
    }

    func testParse_URLWithEncodedPath_UniversalLink() throws {
        // Given - path with URL-encoded characters
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/test/results/123%00456"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        // URL with null byte should be handled gracefully
        XCTAssertEqual(result, .invalid, "should reject IDs with encoded null bytes")
    }

    func testParse_URLWithPlusSign_URLScheme() throws {
        // Given - ID with plus sign (sometimes used as space encoding)
        let url = try XCTUnwrap(URL(string: "aiq://test/results/123+456"))

        // When
        let result = sut.parseDeepLink(url)

        // Then
        XCTAssertEqual(result, .invalid, "should reject IDs with plus signs")
    }

    // MARK: - DeepLink Equality Tests

    func testDeepLink_Equality_TestResults_SameID() {
        // Given
        let link1 = DeepLink.testResults(id: 100)
        let link2 = DeepLink.testResults(id: 100)

        // Then
        XCTAssertEqual(link1, link2, "test results with same ID should be equal")
    }

    func testDeepLink_Equality_TestResults_DifferentID() {
        // Given
        let link1 = DeepLink.testResults(id: 100)
        let link2 = DeepLink.testResults(id: 200)

        // Then
        XCTAssertNotEqual(link1, link2, "test results with different IDs should not be equal")
    }

    func testDeepLink_Equality_ResumeTest_SameSessionID() {
        // Given
        let link1 = DeepLink.resumeTest(sessionId: 50)
        let link2 = DeepLink.resumeTest(sessionId: 50)

        // Then
        XCTAssertEqual(link1, link2, "resume test with same session ID should be equal")
    }

    func testDeepLink_Equality_ResumeTest_DifferentSessionID() {
        // Given
        let link1 = DeepLink.resumeTest(sessionId: 50)
        let link2 = DeepLink.resumeTest(sessionId: 75)

        // Then
        XCTAssertNotEqual(link1, link2, "resume test with different session IDs should not be equal")
    }

    func testDeepLink_Equality_Settings() {
        // Given
        let link1 = DeepLink.settings
        let link2 = DeepLink.settings

        // Then
        XCTAssertEqual(link1, link2, "settings links should be equal")
    }

    func testDeepLink_Equality_Invalid() {
        // Given
        let link1 = DeepLink.invalid
        let link2 = DeepLink.invalid

        // Then
        XCTAssertEqual(link1, link2, "invalid links should be equal")
    }

    func testDeepLink_Equality_DifferentTypes() {
        // Given
        let link1 = DeepLink.testResults(id: 100)
        let link2 = DeepLink.settings
        let link3 = DeepLink.invalid

        // Then
        XCTAssertNotEqual(link1, link2, "different link types should not be equal")
        XCTAssertNotEqual(link1, link3, "different link types should not be equal")
        XCTAssertNotEqual(link2, link3, "different link types should not be equal")
    }

    // MARK: - Case Sensitivity Tests

    func testParse_CaseSensitive_Host_URLScheme() throws {
        // Given - URL schemes are case-insensitive for the scheme, but host is treated as path
        let urlLower = try XCTUnwrap(URL(string: "aiq://test/results/123"))
        let urlUpper = try XCTUnwrap(URL(string: "aiq://TEST/results/123"))

        // When
        let resultLower = sut.parseDeepLink(urlLower)
        let resultUpper = sut.parseDeepLink(urlUpper)

        // Then
        XCTAssertEqual(resultLower, .testResults(id: 123), "lowercase should work")
        XCTAssertEqual(resultUpper, .invalid, "uppercase host should be invalid")
    }

    func testParse_CaseSensitive_Path_URLScheme() throws {
        // Given
        let urlLower = try XCTUnwrap(URL(string: "aiq://test/results/123"))
        let urlMixed = try XCTUnwrap(URL(string: "aiq://test/Results/123"))

        // When
        let resultLower = sut.parseDeepLink(urlLower)
        let resultMixed = sut.parseDeepLink(urlMixed)

        // Then
        XCTAssertEqual(resultLower, .testResults(id: 123), "lowercase path should work")
        XCTAssertEqual(resultMixed, .invalid, "mixed case path should be invalid")
    }

    func testParse_CaseSensitive_UniversalLink() throws {
        // Given
        let urlLower = try XCTUnwrap(URL(string: "https://a-iq-test.com/test/results/123"))
        let urlMixed = try XCTUnwrap(URL(string: "https://a-iq-test.com/Test/Results/123"))

        // When
        let resultLower = sut.parseDeepLink(urlLower)
        let resultMixed = sut.parseDeepLink(urlMixed)

        // Then
        XCTAssertEqual(resultLower, .testResults(id: 123), "lowercase path should work")
        XCTAssertEqual(resultMixed, .invalid, "mixed case path should be invalid")
    }

    // MARK: - Integration Tests

    func testParse_AllSupportedRoutes_URLScheme() {
        // Test all supported URL scheme routes
        let testCases: [(String, DeepLink)] = [
            ("aiq://test/results/1", .testResults(id: 1)),
            ("aiq://test/results/999", .testResults(id: 999)),
            ("aiq://test/resume/50", .resumeTest(sessionId: 50)),
            ("aiq://test/resume/123", .resumeTest(sessionId: 123)),
            ("aiq://settings", .settings)
        ]

        for (urlString, expected) in testCases {
            guard let url = URL(string: urlString) else {
                XCTFail("Failed to create URL from \(urlString)")
                continue
            }

            let result = sut.parseDeepLink(url)
            XCTAssertEqual(result, expected, "Failed to parse \(urlString)")
        }
    }

    func testParse_AllSupportedRoutes_UniversalLink() {
        // Test all supported universal link routes (production domain)
        let testCases: [(String, DeepLink)] = [
            ("https://a-iq-test.com/test/results/1", .testResults(id: 1)),
            ("https://a-iq-test.com/test/results/999", .testResults(id: 999)),
            ("https://a-iq-test.com/test/resume/50", .resumeTest(sessionId: 50)),
            ("https://a-iq-test.com/test/resume/123", .resumeTest(sessionId: 123)),
            ("https://a-iq-test.com/settings", .settings)
        ]

        for (urlString, expected) in testCases {
            guard let url = URL(string: urlString) else {
                XCTFail("Failed to create URL from \(urlString)")
                continue
            }

            let result = sut.parseDeepLink(url)
            XCTAssertEqual(result, expected, "Failed to parse \(urlString)")
        }
    }

    func testParse_AllSupportedRoutes_UniversalLink_DevDomain() {
        // Test all supported universal link routes (development domain)
        let testCases: [(String, DeepLink)] = [
            ("https://dev.a-iq-test.com/test/results/1", .testResults(id: 1)),
            ("https://dev.a-iq-test.com/test/results/999", .testResults(id: 999)),
            ("https://dev.a-iq-test.com/test/resume/50", .resumeTest(sessionId: 50)),
            ("https://dev.a-iq-test.com/test/resume/123", .resumeTest(sessionId: 123)),
            ("https://dev.a-iq-test.com/settings", .settings)
        ]

        for (urlString, expected) in testCases {
            guard let url = URL(string: urlString) else {
                XCTFail("Failed to create URL from \(urlString)")
                continue
            }

            let result = sut.parseDeepLink(url)
            XCTAssertEqual(result, expected, "Failed to parse dev domain URL: \(urlString)")
        }
    }

    func testParse_URLSchemeAndUniversalLink_Equivalence() {
        // Verify that URL scheme and universal link produce same result
        let equivalentPairs: [(String, String)] = [
            ("aiq://test/results/123", "https://a-iq-test.com/test/results/123"),
            ("aiq://test/resume/456", "https://a-iq-test.com/test/resume/456"),
            ("aiq://settings", "https://a-iq-test.com/settings")
        ]

        for (urlScheme, universalLink) in equivalentPairs {
            guard let url1 = URL(string: urlScheme),
                  let url2 = URL(string: universalLink) else {
                XCTFail("Failed to create URLs")
                continue
            }

            let result1 = sut.parseDeepLink(url1)
            let result2 = sut.parseDeepLink(url2)

            XCTAssertEqual(result1, result2, "URL scheme \(urlScheme) and universal link \(universalLink) should produce same result")
        }
    }

    // MARK: - DeepLinkParser Protocol Conformance Tests

    /// Test that the DeepLinkParser.parse(url:) method returns correct actions for resume test
    func testParseAction_ResumeTest_ReturnsPopToRootThenPush() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/resume/123"))

        // When
        let action = sut.parse(url: url)

        // Then
        if case let .popToRootThenPush(route) = action {
            XCTAssertEqual(route, .testTaking(sessionId: 123), "should return testTaking route with sessionId")
        } else {
            XCTFail("Expected .popToRootThenPush action, got \(String(describing: action))")
        }
    }

    /// Test that the DeepLinkParser.parse(url:) returns nil for settings (handled at tab level)
    func testParseAction_Settings_ReturnsNil() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://settings"))

        // When
        let action = sut.parse(url: url)

        // Then
        XCTAssertNil(action, "settings should return nil (handled at tab level)")
    }

    /// Test that the DeepLinkParser.parse(url:) returns nil for test results (needs async API call)
    func testParseAction_TestResults_ReturnsNil() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "aiq://test/results/123"))

        // When
        let action = sut.parse(url: url)

        // Then
        XCTAssertNil(action, "test results should return nil (needs async API call)")
    }

    /// Test that the DeepLinkParser.parse(url:) returns nil for invalid URLs
    func testParseAction_Invalid_ReturnsNil() throws {
        // Given
        let url = try XCTUnwrap(URL(string: "custom://unknown"))

        // When
        let action = sut.parse(url: url)

        // Then
        XCTAssertNil(action, "invalid URLs should return nil")
    }

    // MARK: - DeepLink Analytics Destination Type Tests

    func testDeepLink_AnalyticsDestinationType_TestResults() {
        // Given
        let deepLink = DeepLink.testResults(id: 123)

        // Then
        XCTAssertEqual(deepLink.analyticsDestinationType, "test_results")
    }

    func testDeepLink_AnalyticsDestinationType_ResumeTest() {
        // Given
        let deepLink = DeepLink.resumeTest(sessionId: 456)

        // Then
        XCTAssertEqual(deepLink.analyticsDestinationType, "resume_test")
    }

    func testDeepLink_AnalyticsDestinationType_Settings() {
        // Given
        let deepLink = DeepLink.settings

        // Then
        XCTAssertEqual(deepLink.analyticsDestinationType, "settings")
    }

    func testDeepLink_AnalyticsDestinationType_Invalid() {
        // Given
        let deepLink = DeepLink.invalid

        // Then
        XCTAssertEqual(deepLink.analyticsDestinationType, "invalid")
    }

    // MARK: - DeepLinkSource Tests

    func testDeepLinkSource_RawValues() {
        // Verify all source raw values are correctly defined
        XCTAssertEqual(DeepLinkSource.pushNotification.rawValue, "push_notification")
        XCTAssertEqual(DeepLinkSource.externalApp.rawValue, "external_app")
        XCTAssertEqual(DeepLinkSource.safari.rawValue, "safari")
        XCTAssertEqual(DeepLinkSource.urlScheme.rawValue, "url_scheme")
        XCTAssertEqual(DeepLinkSource.universalLink.rawValue, "universal_link")
        XCTAssertEqual(DeepLinkSource.unknown.rawValue, "unknown")
    }

    // MARK: - Analytics Tracking Tests

    /// Test that malformed settings deep links track analytics for extra path components
    func testParseSettings_WithExtraComponents_TracksAnalytics() throws {
        // Given - URL with extra path components after settings
        let url = try XCTUnwrap(URL(string: "aiq://settings/notifications"))

        // When - parsing the URL
        let result = sut.parseDeepLink(url)

        // Then - should parse as .settings (lenient behavior)
        XCTAssertEqual(result, .settings, "should parse as settings despite extra components")

        // Then - should track analytics for malformed deep link
        XCTAssertTrue(mockAnalytics.trackDeepLinkFailedCalled, "should track failed navigation for malformed settings")
        XCTAssertEqual(
            mockAnalytics.lastFailedErrorType,
            "malformed_settings_extra_components",
            "should use correct error type"
        )
        XCTAssertEqual(mockAnalytics.lastFailedSource, "unknown", "should use unknown source for parsing errors")
        XCTAssertEqual(mockAnalytics.lastFailedURL, url.absoluteString, "should track original URL")
    }

    /// Test that malformed settings deep links with multiple extra components track analytics
    func testParseSettings_WithMultipleExtraComponents_TracksAnalytics() throws {
        // Given - URL with multiple extra path components
        let url = try XCTUnwrap(URL(string: "https://a-iq-test.com/settings/notifications/push/detail"))

        // When - parsing the URL
        let result = sut.parseDeepLink(url)

        // Then - should parse as .settings
        XCTAssertEqual(result, .settings, "should parse as settings despite multiple extra components")

        // Then - should track analytics
        XCTAssertTrue(mockAnalytics.trackDeepLinkFailedCalled, "should track failed navigation")
        XCTAssertEqual(
            mockAnalytics.lastFailedErrorType,
            "malformed_settings_extra_components",
            "should use correct error type for multiple components"
        )
    }

    /// Test that well-formed settings deep links do NOT track failure analytics
    func testParseSettings_Valid_DoesNotTrackFailureAnalytics() throws {
        // Given - Valid settings URL without extra components
        let url = try XCTUnwrap(URL(string: "aiq://settings"))

        // When - parsing the URL
        let result = sut.parseDeepLink(url)

        // Then - should parse as .settings
        XCTAssertEqual(result, .settings, "should parse as settings")

        // Then - should NOT track failure analytics (no malformation)
        XCTAssertFalse(
            mockAnalytics.trackDeepLinkFailedCalled,
            "should not track failure for well-formed settings URL"
        )
    }

    /// Test that settings deep link with trailing slash does NOT track failure analytics
    func testParseSettings_WithTrailingSlash_DoesNotTrackFailureAnalytics() throws {
        // Given - Settings URL with trailing slash (still well-formed)
        let url = try XCTUnwrap(URL(string: "aiq://settings/"))

        // When - parsing the URL
        let result = sut.parseDeepLink(url)

        // Then - should parse as .settings
        XCTAssertEqual(result, .settings, "should parse as settings")

        // Then - should NOT track failure analytics (trailing slash is not an extra component)
        XCTAssertFalse(
            mockAnalytics.trackDeepLinkFailedCalled,
            "should not track failure for settings URL with trailing slash"
        )
    }
}
