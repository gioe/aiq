...
**Implementation Notes:**
Comprehensive unit tests added in commit d2faf41. The test file includes 86 unit tests covering all core functions.
...
**Implementation Notes:**
Created `backend/tests/test_distractor_endpoints.py` with 21 integration tests covering:
- TestSingleQuestionDistractorAnalysisIntegration (7 tests): Simulated responses, quartile data, insufficient responses, 404, free-response error, auth
- TestBulkDistractorSummaryIntegration (9 tests): Multiple questions, type filter, empty results, offenders structure, breakdowns, auth
- TestDistractorEndpointSchemas (2 tests): Response field validation
- TestKnownDataPatterns (4 tests): Correct answer identification, selection rates, effective option count, guessing probability

**Acceptance Criteria:**
- [x] Both endpoints have integration tests
- [x] Test authentication requirements
- [x] Test with known data patterns
