...
| `ios/scripts/publish_api_client.sh` | Publish updated spec to `gioe/ios-libs` for a new remote package release |
...
1. **Publish script**: `ios/scripts/publish_api_client.sh` copies spec to a local ios-libs checkout and prompts to publish a new release
2. **Code generation**: Swift OpenAPI Generator plugin creates type-safe client code from the published package
3. **Build verification**: Build fails if the package version is out of sync or has breaking changes

**To publish an updated spec locally:**
...
## Publishing an Updated iOS Client

When the OpenAPI spec changes, publish a new release to `gioe/ios-libs`:

```bash
./ios/scripts/publish_api_client.sh ~/code/ios-libs
```

The script copies `docs/api/openapi.json` to `Sources/APIClient/openapi.json` in the local checkout and walks you through committing and tagging a new semver release.
...
### Scenario: "iOS build fails because the API client is outdated"

**Fix:** Publish a new ios-libs release with the latest spec:
```bash
./ios/scripts/publish_api_client.sh ~/code/ios-libs
# Then update the ios-libs version in the Xcode project and rebuild
```
