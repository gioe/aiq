#!/bin/bash
# Cron wrapper script for AIQ Backend CAT Readiness
#
# Derived from libs/templates/run_cron.sh.template.
# See libs/templates/README.md for the 5-step setup workflow.
#
# In production this job is scheduled via Railway (backend/infra/railway-cron-readiness.json).
# Use this script for local scheduling or non-Railway environments.
#
# Usage:
#   Add to crontab:
#   30 3 * * * /path/to/aiq/backend/scripts/run_cron_readiness.sh >> /path/to/logs/cron.log 2>&1

# ── Configure these four values ──────────────────────────────────────────────
SERVICE_NAME="AIQ Backend CAT Readiness"
REQUIRED_ENV_VARS="DATABASE_URL"
SCRIPT_PATH="scripts/run_cat_readiness.py"
PYTHON_ARGS=""
# ─────────────────────────────────────────────────────────────────────────────
