...
async def request_password_reset(
    ...
    try:
        ...
        if user:
            ...
            active_token_count = result.scalar_one()

            if active_token_count >= MAX_TOKENS_PER_USER:  # type: ignore[operator]
                logger.warning(
            ...
            while True:
                ...
                tokens_to_invalidate = result.all()

                if not tokens_to_invalidate:
                    break

                token_ids = [t[0] for t in tokens_to_invalidate]
                await db.execute(
