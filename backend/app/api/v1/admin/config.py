...
async def get_weighted_scoring_status(
    ...
    """
    try:
        enabled = await is_weighted_scoring_enabled(db)
        weights = await get_domain_weights(db)

        # Get the updated_at timestamp if the config exists
        config_record = await get_config(db, "use_weighted_scoring")
        updated_at = None
        if config_record:
            ...
            result = await db.execute(
                select(SystemConfig).filter(SystemConfig.key == "use_weighted_scoring")
            )
...
async def toggle_weighted_scoring(
    ...
    """
    try:
        result = await set_weighted_scoring_enabled(db, request.enabled)
...
async def get_domain_weights_config(
    ...
    try:
        ...
        result = await db.execute(
            select(SystemConfig).filter(SystemConfig.key == "domain_weights")
        )
        record = result.scalars().first()
...
async def set_domain_weights_config(
    ...
    try:
        ...
        result = await set_domain_weights(db, request.weights)
