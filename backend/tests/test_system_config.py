...
class TestSystemConfigModel:
    ...
    async def test_value_accepts_various_json_types(self, db_session):
        ...
        for key, expected_value in test_cases:
            _qresult = await db_session.execute(
                select(SystemConfig).filter(SystemConfig.key == key)
            )

            config = _qresult.scalars().first()
...
class TestGetConfig:
    ...
    async def test_get_existing_config(self, db_session):
        ...
        result = await get_config(db_session, "existing_key")
        assert result == {"data": "test"}

    async def test_get_nonexistent_config_returns_none(self, db_session):
        """Test getting a non-existent key returns None by default."""
        result = await get_config(db_session, "nonexistent_key")
        assert result is None

    async def test_get_nonexistent_config_returns_default(self, db_session):
        """Test getting a non-existent key returns the provided default."""
        default = {"default": "value"}
        result = await get_config(db_session, "nonexistent_key", default=default)
...
class TestSetConfig:
    ...
    async def test_set_new_config(self, db_session):
        """Test setting a new configuration value."""
        result = await set_config(db_session, "new_key", {"new": "value"})
        ...
        _qresult = await db_session.execute(
            select(SystemConfig).filter(SystemConfig.key == "new_key")
        )
    ...
    async def test_update_existing_config(self, db_session):
        ...
        # Update the config
        result = await set_config(db_session, "update_key", {"updated": True})
...
class TestDeleteConfig:
    ...
    async def test_delete_existing_config(self, db_session):
        ...
        result = await delete_config(db_session, "delete_key")
        assert result is True

        # Verify it was deleted
        _qresult = await db_session.execute(
            select(SystemConfig).filter(SystemConfig.key == "delete_key")
        )
    ...
    async def test_delete_nonexistent_config(self, db_session):
        ...
        result = await delete_config(db_session, "nonexistent_key")
...
class TestGetAllConfigs:
    ...
    async def test_get_all_configs_empty(self, db_session):
        """Test getting all configs when none exist."""
        result = await get_all_configs(db_session)
        assert result == {}

    async def test_get_all_configs_multiple(self, db_session):
...
class TestConfigExists:
    ...
    async def test_config_exists_true(self, db_session):
        """Test that config_exists returns True for existing key."""
        config = SystemConfig(key="exists_key", value="exists", updated_at=utc_now())
        db_session.add(config)
        await db_session.commit()

        assert await config_exists(db_session, "exists_key") is True

    async def test_config_exists_false(self, db_session):
        """Test that config_exists returns False for non-existing key."""
        assert await config_exists(db_session, "nonexistent_key") is False


class TestDomainWeightsHelpers:
    """Tests for domain weights convenience functions."""

    async def test_get_domain_weights_not_set(self, db_session):
        """Test getting domain weights when not configured."""
        result = await get_domain_weights(db_session)
        assert result is None

    async def test_set_and_get_domain_weights(self, db_session):
        ...
        await set_domain_weights(db_session, weights)
        result = await get_domain_weights(db_session)
...
class TestWeightedScoringHelpers:
    ...
    async def test_weighted_scoring_default_disabled(self, db_session):
        """Test that weighted scoring is disabled by default."""
        result = await is_weighted_scoring_enabled(db_session)
        assert result is False

    async def test_enable_weighted_scoring(self, db_session):
        """Test enabling weighted scoring."""
        await set_weighted_scoring_enabled(db_session, True)
        result = await is_weighted_scoring_enabled(db_session)
        assert result is True
    ...
    async def test_disable_weighted_scoring(self, db_session):
        ...
        await set_weighted_scoring_enabled(db_session, True)
        await set_weighted_scoring_enabled(db_session, False)
        result = await is_weighted_scoring_enabled(db_session)
        assert result is False
...
class TestDomainPopulationStatsHelpers:
    ...
    async def test_get_population_stats_not_set(self, db_session):
        """Test getting population stats when not configured."""
        result = await get_domain_population_stats(db_session)
        assert result is None

    async def test_set_and_get_population_stats(self, db_session):
        ...
        await set_domain_population_stats(db_session, stats)
        result = await get_domain_population_stats(db_session)
        assert result == stats
