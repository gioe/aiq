...
class TestCompleteUserJourney:
    ...
    async def test_complete_new_user_journey(self, client, test_questions):
        """
        ...
        register_response = await client.post(
            "/v1/auth/register", json=register_payload
        )
        ...
        active_response = await client.get("/v1/test/active", headers=auth_headers)
        ...
        # Note: Answers must match the correct_answer field value, not the option label
        # Map question IDs to correct answers based on the test fixture
        # Fixture questions: pattern ("10"), logic ("No"), math ("180"), verbal ("scarce")
        answer_mapping = {
            "2, 4, 6, 8": "10",  # Pattern question
            "roses are flowers": "No",  # Logic question
            "15 * 12": "180",  # Math question
            "antonym": "scarce",  # Verbal question
        }

        # Build responses: answer first 2 correctly, third incorrectly
        responses = []
        for i, question in enumerate(questions):
            q_text = question["question_text"]
            # Find the correct answer based on question text
            correct_answer = next(
                (ans for key, ans in answer_mapping.items() if key in q_text), "unknown"
            )
            # Answer first 2 correctly, third incorrectly
            user_answer = correct_answer if i < 2 else "wrong_answer"
            responses.append(
                {
        ...
        active_final = await client.get("/v1/test/active", headers=auth_headers)
        assert active_final.status_code == 200
        assert active_final.json() is None  # No active session after completion

    async def test_multiple_tests_over_time(self, client, test_questions, db_session):
        """
            ...
            "responses": [
                {
                    ...
                    "user_answer": "10",  # Correct answer
                },
                {
                    "question_id": questions1[1]["id"],
                    "user_answer": "No",  # Correct answer
                },
            ],
        }
        submit1 = await client.post(
            "/v1/test/submit", json=responses1, headers=auth_headers
        )
        ...
        _qresult = await db_session.execute(
            select(TestResultModel).filter(
                TestResultModel.test_session_id == session1_id
            )
        )
        ...
        test_session = _qresult.scalars().first()
            ...
            "responses": [
                {
                    ...
                    "user_answer": "180",  # Correct answer for "15 * 12 = ?"
                },
                {
                    "question_id": questions2[1]["id"],
                    "user_answer": "scarce",  # Correct answer for antonym question
                },
            ],
        }
        submit2 = await client.post(
            "/v1/test/submit", json=responses2, headers=auth_headers
        )
...
class TestNotificationIntegrationWithTests:
    ...
    async def test_user_test_completion_updates_eligibility(
...
class TestAuthenticationFlow:
    ...
    async def test_unauthorized_access_attempts(self, client, test_questions):
        """
        ...
        response = await client.get("/v1/user/profile", headers=invalid_headers)
...
class TestErrorRecovery:
    ...
    async def test_abandoned_test_recovery(self, client, test_questions):
        """
