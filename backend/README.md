...
**Important:** The `docs/api/openapi.json` file is **auto-generated by CI** and should NOT be manually edited. Any manual changes will be overwritten on the next backend push to `main`.
...
To manually generate or validate the OpenAPI spec:

```bash
# Generate spec (default location: docs/api/openapi.json)
python export_openapi.py

# Generate with validation
python export_openapi.py --validate

# Generate without transforms (raw OpenAPI 3.1)
...
- Required OpenAPI fields are present
- Schema structure is well-formed
- All `$ref` references resolve correctly

See [docs/OPENAPI_USAGE_GUIDE.md](../docs/OPENAPI_USAGE_GUIDE.md) for detailed usage information.
